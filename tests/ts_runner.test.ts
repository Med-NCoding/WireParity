/**
 * Tests for Step 3.3: TypeScript SDK Runner Worker
 *
 * Verifies that fixtures/runners/ts_runner.ts executed as a real subprocess via SubprocessSDKRunner:
 * 1. Correctly receives IPCRequest over stdin.
 * 2. Translates OperationInputs into TypeScript SDK method invocations.
...
describe("TypeScript SDK Runner Worker (ts_runner.ts)", () => {
  let server: CaptureServer;
  let runner: SubprocessSDKRunner;

  beforeEach(async () => {
    server = await startCaptureServer();
    runner = new SubprocessSDKRunner("typescript", {
